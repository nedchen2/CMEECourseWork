...
neutral_step_speciation <- function(community,speciation_rate)  {
  ...
  if (runif(1,min  = 0, max = 1) <= speciation_rate){
    community[index1] <- max(community) + 1
  }else{
    community <- neutral_step(community)
  }
  return(community)
...
question_12 <- function()  {
  ...
  lines(richness2,col="red")
  legend_names <- c("Max initialized richness","Min initialized richness")
  legend("topright", legend_names,
         col = c("blue", "red"), lty = 1, bty = "n")
  return("Regardless of the initialized richness status, the richness tends to converge to the fluctation curve in a similar constraints range. This is because of the dynamic equilibrium between speciation and extinction caused by random factor")
}
...
question_16 <- function()  {
  ...
  final_frame <-rbind(octaves_max_final,octaves_min_final)
  test1 <- as.data.frame(final_frame)
...
cluster_run <- function(speciation_rate, size, wall_time, interval_rich, interval_oct, burn_in_generations, output_file_name)  {
  ...
  octaves_list <- list(octaves(species_abundance(community))) # predefined
  
  count_of_generation <- 0 # predefined 
  ...
  while (unname((proc.time() - ptm)[3]) <= 60 * wall_time){ # limit the time running # while loop work better than for
    ...
    if (count_of_generation %% interval_oct == 0){ # the index of the list is the actual generation number
...
function_for_process20 <- function(range){
  ...
  for (i in range){
    ...
    for (index in seq(length(octaves_list))){
      generations = (index -1) * interval_oct
      if (generations > burn_in_generations){
        octaves_list_post_burn_in <- octaves_list[[index]]
...
ploting_for_question20 <- function(data,size,lab){
...
spiral <- function(start_position, direction, length)  {
  ...
  spiral(first_end_point, direction - pi/4 , length*0.95)
  }
...
tree <- function(start_position, direction, length,threshold = 0.01)  {
  if (length >= threshold){
    first_end_point <- turtle(start_position,direction,length) #get the first line
    tree(first_end_point, direction + pi/4 , length*0.65)
    tree(first_end_point, direction - pi/4 , length*0.65)
  }
}
...
fern <- function(start_position, direction, length)  {
  if (length >= 0.005){
    first_end_point <- turtle(start_position,direction,length) #get the first line
    fern(first_end_point, direction + pi/4 , length*0.38)
    fern(first_end_point, direction , length*0.87)
  }
}
...
Challenge_A <- function() { # this question is about the species in 
  ...
  segments(x0 = i, y0 = 0, x1 = i, y1 = 100, col = "steelblue")#Green line indicates when equilibrium reached
...
Challenge_B <- function() {
  ...
  for (i in test_vector){ # have some sample richness
    ...
    Richness_mean = apply(Richness_matrix,2,mean)
    lines(Richness_mean,col=col_vector[location])
...
process_result_for_ChallengeC <- function(range){ 
  ...
  mean_average <- richness_sum/counts_of_repeats
  return(mean_average)
}

Challenge_C <- function() {
  graphics.off()# clear any existing graphs and plot your graph within the R window
  mean_richness_500 <- process_result_for_ChallengeC(1:25)
...
Challenge_D <- function() {
  ...
  octaves_mean_500 <- octaves_sum_500/25
...
tree2 <- function(start_position, direction, length,threshold = 0.01,col="red",lwd = 5)  {
  if (length >= threshold){
    ...
    lwd <- turtle2(start_position,direction,length,col="red",lwd)[[2]]
    col <- turtle2(start_position,direction,length,col="red",lwd)[[3]]
...
tree3 <- function(start_position, direction, length,threshold = 0.01,col=1,lwd = 5)  {
  if (length >= threshold){
    ...
    lwd <- turtle2(start_position,direction,length,col=col,lwd)[[2]]
    col <- turtle2(start_position,direction,length,col=col,lwd)[[3]]
...
fern4 <- function(start_position, direction, length, dir,col=1,lwd=5)  {
  if (length >= 0.01){
    first_end_point <- turtle2(start_position,direction,length,col=col,lwd)[[1]]
    lwd <- turtle2(start_position,direction,length,col=col,lwd)[[2]]
    col <- turtle2(start_position,direction,length,col=col,lwd)[[3]]#get the first line
