...
plot(density(genome$GenomeSize, bw=0.01))


# na.rm magic!
plot(density(genome$TotalLength, bw=5, na.rm=TRUE))

# change the breaks and bw will change the smooth of the graph
######################## (5) ##########################
# COMPARE TWO DISTRIBUTIONS OF FACTORS OF INTEREST USING BOXPLOTS:

plot(GenomeSize ~ Suborder, data=genome)  #plot function gives box plot when explanatory variables is a factor
...
lines(density(Anisoptera$GenomeSize), col='red') #can be used to direct add the line to current plot
...
#when the variables is the same continuous variables, the plot function will give scatter 
...
#for a categorical factor R will give index to them, which could be used to plot
#the Suborder shows the index;for example the first one is 1
plot(GenomeSize ~ TotalLength , data = genome, #Now plot again
	col = myColours[Suborder], pch = mySymbols[Suborder], #pch means plot character 
	xlab='Total length (mm)', ylab='Genome size (pg)')

#To the legend function we can also take advantage of the levels of the factor
