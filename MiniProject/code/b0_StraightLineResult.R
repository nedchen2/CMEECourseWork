#Author :Congjia.chen
#Function : straightline model fitting and AIC BIC CALCULATION
#Input ../results/Csvafterpreprocessing2.csv
#Output ../results/straightLine_result.csv
#dep: tidyverse


# Basic approach exponential growth AND OLS approach
# Nt = N0e^(rt)
# after log transformation: log(Nt) = log(N0) + rt
...
colnames(df) <- gsub("\\(Intercept\\)","intercept",colname2) #remove the space in the colnames

write.csv(df,"../results/straightLine_result.csv",row.names = F)

print ("=========================StraightLine model fitting done===========================")