#Author :Congjia.chen
#Function : Quadratic Polynomial model fitting and AIC BIC CALCULATION
#Input ../results/Csvafterpreprocessing2.csv
#Output ../results/QuadraticPloynomial_result.csv
#dep: tidyverse, minpack.lm

# quadratic
# linear-OLS- polynomial model fitting
...
for (i in seq(length(unique(data_model2$ID)))){
  ...
  fit_model <- try(lm(data = df_subset,
                      LogPopBio ~ poly(Time,2,raw = T)),silent = T) 
...
colnames(df) <- gsub(" ","",gsub("poly\\(Time, 2, raw = T\\)","A",gsub("\\(Intercept\\)","A0",colname2))) #remove the space in the colnames

write.csv(df,"../results/QuadraticPloynomial_result.csv",row.names = F)

print ("=========================QuaPolynomial growth model fitting done===========================")
